...
def get_U(q):
    ...
    for i in range(N_alpha):
        ...
        U_11[i, i] = WignerD(sp.Rational(1/2), sp.Rational(1/2),
                             sp.Rational(1/2), phi[i_phi], theta[i_theta], psi[i_psi]).doit()
        U_12[i, i] = WignerD(sp.Rational(1/2), sp.Rational(1/2),
                             sp.Rational(-1/2), phi[i_phi], theta[i_theta], psi[i_psi]).doit()
        U_21[i, i] = WignerD(sp.Rational(1/2), sp.Rational(-1/2),
                             sp.Rational(1/2), phi[i_phi], theta[i_theta], psi[i_psi]).doit()
        U_22[i, i] = WignerD(sp.Rational(1/2), sp.Rational(-1/2),
                             sp.Rational(-1/2), phi[i_phi], theta[i_theta], psi[i_psi]).doit()
    ####
